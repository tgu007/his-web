...
@Component({
  selector: 'app-internal-auto-fee-detail',
  templateUrl: './internal-auto-fee-detail.component.html',
  styleUrls: ['./internal-auto-fee-detail.component.css']
})
export class InternalAutoFeeDetailComponent implements OnInit {
  itemList: any;
  isSaving: any = false;
  ...
  saveAutoFee() {
    ...
    this.isSaving = true;
    this.internalAccountService.saveAutoFee(autoFee)
      .subscribe(response => {
          this.isSaving = false;
          if (response) {
  ...
  private patchFormValue() {
    this.autoFeeDetailForm.patchValue({
      ...
      txtPatientInfo: this.autoFee.patientInfo,
      txtSignInNumber: this.autoFee.singInNumber,
    });
  }
